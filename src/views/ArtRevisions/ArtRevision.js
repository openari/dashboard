...
import { getArtRevision, approveArtRevision, identificationColumnNameLookup, statusNameLookup } from '../../services/ArtRevisionsService';
...
class ArtRevision extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: {},
    ...
    getArtRevision(getToken(), this.props.match.params.id)
    .then(data => {
  ...
  confirmApprove = (e) => {
    this.setState({
      ...
      approveArtRevision(getToken(), this.props.match.params.id)
      .then(() => {
        this.setState({
      ...
      <div className="animated fadeIn">
        ...
        <Row>
          <Col lg={8}>
            <Card>
              <CardHeader>
                <strong><i className="icon-info pr-1"></i>作品異動 Id: {this.props.match.params.id}</strong>
                    ...
                    <tbody>
                      {
                        data.map(([key, value], index) => {
                          if (key == 'title' || key == 'subject' ||
                              key == 'identification' || key == 'ownership') {
                            return;
                          } else if (key === 'art_id') {
                            return (
