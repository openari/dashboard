...
import { listApplications, statusNameLookup } from '../../services/ApplicationsService';
...
class Applications extends Component {
  ...
  loadData = () => {
    listApplications(getToken(), this.state.status)
    .then((data) => {
  ...
  render() {
    ...
    let itemIdFormatter = (cell, row) => {
      const itemLink = `#/applications/${row.id}`;
        ...
        <Card>
          ...
          <CardBody>
            <BootstrapTable data={this.state.table} version="4" striped hover pagination search options={this.options}>
              ...
              <TableHeaderColumn dataField="applicant" dataFormat={itemApplicantFormatter} dataSort>申請人/單位</TableHeaderColumn>
